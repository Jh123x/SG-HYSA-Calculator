...
const new_ir_cutoff: Array<Interest> = [
    // Spend 500 + Salary Credit
    {
        cutoffs: [
...
export const uob_new_interest = (profile: Profile): ResultInterest => {
    const { Savings, Spending, GiroTransactions, Salary } = profile

    if (Spending >= 500 && Salary >= 1600) {
        return calculate_ir(Savings, new_ir_cutoff[0])
    }

    if (Spending >= 500 && GiroTransactions >= 3) {
        return calculate_ir(Savings, new_ir_cutoff[1])
    }
    ...
    return calculate_ir(Savings, new_ir_cutoff[3])
}
